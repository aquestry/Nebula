...
public class ServerPostConnect {
    public ServerPostConnect(ServerPostConnectEvent event) {
        ...
        String hubName = Nebula.util.getBackendServer(server.getServerInfo().getName())
                .getHoldServer()
                .getServerName();
        ...
        player.sendPlayerListHeaderAndFooter(Nebula.mm.deserialize("<blue><bold>Nebula</bold></blue>\nPlayers on network: " + Nebula.server.getAllPlayers().size() + "/" + Nebula.server.getConfiguration().getShowMaxPlayers())
                ,Nebula.mm.deserialize("Your ping is: " + player.getPing() + "ms"));
    }
}