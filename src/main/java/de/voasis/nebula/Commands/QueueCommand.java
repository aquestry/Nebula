        ...
        Data.gamemodeQueueMap.stream()
                ...
                .ifPresentOrElse(
                        queue -> {
                            ...
                            player.sendMessage(mm.deserialize(Messages.FEEDBACK_ADDED_TO_QUEUE.replace("<queue>", queueName)));
                        },
                        () -> player.sendMessage(mm.deserialize(Messages.FEEDBACK_QUEUE_NOT_FOUND))
                );
    }

    private void leaveQueue(Player player) {
        if (!isInAnyQueue(player)) {
            player.sendMessage(mm.deserialize(Messages.FEEDBACK_NOT_IN_QUEUE));
            return;
        }
        Data.gamemodeQueueMap.stream()
                ...
                .forEach(queue -> {
                    ...
                    player.sendMessage(mm.deserialize(Messages.FEEDBACK_REMOVED_FROM_QUEUE.replace("<queue>", queue.getName())));
                });
    }
}
